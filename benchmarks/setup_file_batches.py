...
def setup_batch(base_dir, number_of_directories, number_of_files, lower_size_bound, upper_size_bound):
    dir_name_pattern = "batch_dir"
    for i in range(number_of_directories):
        subdir_name = os.path.join(base_dir, dir_name_pattern)
        dir_path = subdir_name + f'_{(i+1)}'
        if os.path.exists(dir_path):
            continue
        # Produce subdirectory with files of equal size
        produce_dir(dir_path, number_of_files, lower_size_bound, upper_size_bound)
    return
...
def main():
    ...
    setup_batch(base_dir, number_of_directories, number_of_files, lower_size_bound, upper_size_bound)
